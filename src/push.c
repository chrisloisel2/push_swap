...
/*   Updated: 2021/08/26 07:03:08 by lchristo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	speed_push(t_stack *r)
{
	...
	if (r->lenb > 0 && r->lena > 0)
	{
		...
		if (r->lenb > 1 && r->b[r->lenb - 1] < r->b[r->lenb - 2])
			sb(r);
		if (r->b[r->lenb - 1] != r->tl[r->lenmax - 1]
			&& next(r, r->b[r->lenb - 1]) == r->b[0])
			return (rrb(r));
		if (r->lena > 3 && prev(r, r->a[r->lena - 3]) == r->a[r->lena - 1])
			return (sa(r));
		if (r->a[r->lena - 1] > r->tl[0]
			&& prev(r, r->a[r->lena - 1]) == r->a[r->lena - 2])
			return (sa(r));
	}
	return (0);
}
...
void	ft_smart_push(t_lst *s, t_stack *r)
{
	...
	{
		speed_push(r);
		if (r->a[r->lena - 1] >= s->min
			&& r->a[r->lena - 1] <= s->max && r->lenb < i)
			pb(r);
		else
			ft_smart_roll2(s, r);
	}
